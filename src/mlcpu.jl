...
for i = 1:1
    for x in train_loader
        sz = size(x)
        sub_sequence = reshape(x,(1,sz[1],sz[2])) # size(sub_sequence) = (inputDim, fullLen, batchSize)
        # size(src) = (inputDim, seqLen, batchSize)
        # size(trg) = (inputDim, forecastLength, batchSize)
        # size(trg_y) = (inputDim, forecastLength, batchSize)
