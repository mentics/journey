...
SH.combineTo(::Type{Ret}, lms::Coll{LegMeta,4}, forDate::Date, sp::Currency, vtyRatio::Float64=1.0)::Ret = combineRets(tos(Ret, lms, forDate, sp, vtyRatio))
SH.combineTo(::Type{Ret}, lms::Coll{LegMeta,4}, sp::Currency)::Ret = isempty(lms) ? Ret(sp) : combineRets(tos(Ret, lms, minimum(getExpiration, lms), sp, 1.0))
