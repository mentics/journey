...
function loadSaSyms(page)
    ...
    body = """{"filter":{"close":{"gte":0.01,"disabled":false}},"quant_rank":false,"sort":null,"type":"stock","per_page":$(SaSymsPerPage),"page":$(page)}"""
...
function loadQuotes()
    symsAll = filter(x -> !haskey(Quotes, x) && !haskey(QuoteNotFound, x), allSyms())
