...
makeStrat() = TStrat(
    (2,Scoring),
    # Params(C(1000), 0.1, 0.2, 0.025, 0.99, 0.07, 1.0, 0.05, 0.5, 1, 6),
    Params(C(1000), 0.1, 0.2, 0.025, 0.99, 0.07, 1.0, 0.05, 0.5, 1, 6),
    makeCtx(),
)
...
makeCtx() = Context(Vector{Cand{2}}(), Vector{Cand{2}}())
...
function BackTypes.checkExit(params::Params, tradeOpen::TradeBTOpen{2,Scoring}, tim, lmsc, curp)::Union{Nothing,String}
    ...
    if rate >= params.InitTakeRate * ratio
        ...
        if rate >= tradeOpen.extra.rate || !(tim.date == trad.targetDate(tradeOpen) && curp > getStrike(tradeOpen.lms[2]) + 2)
...
function findEntry!(keep, params, prob, search, otoq, args...)
    ...
    lasti = lastindex(oqs)
    for i1 in eachindex(oqs)[1:lasti-1]
        oq1 = oqs[i1]
        str1 = getStrike(oq1)
