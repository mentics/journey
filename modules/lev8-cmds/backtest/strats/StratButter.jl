module StratButter
using Dates
using SH, BaseTypes, SmallTypes, BackTypes, LegQuoteTypes, ProbTypes
...
BackTypes.pricingOpen(::TStrat3, lmso::NTuple{N,LegQuoteOpen}) where N = toPT(price_open(lmso))
BackTypes.pricingClose(::TStrat3, lmsc::NTuple{N,LegQuoteClose}) where N = toPT(price_close(lmsc))
        ...
        for i2 in (i1+1):lastiLeft
            ...
            for i3 in 1:(lastiRight-1)
                ...
                for i4 in (i3+1):lastiRight
                    ...
                    lms = CollUtil.sortuple(x -> getStrike(x) + (isCall(x) ? eps(Currency) : 0.0), LegQuoteOpen(oq1, Side.long), LegQuoteOpen(oq2, Side.short), LegQuoteOpen(oq3, Side.short), LegQuoteOpen(oq4, Side.long))
