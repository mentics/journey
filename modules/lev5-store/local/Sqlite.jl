...
run(stmt::SQLite.Stmt, params::Coll) = DBInterface.execute(stmt, params; strict=true)
run(stmt::SQLite.Stmt, params...) = DBInterface.execute(stmt, params; strict=true)
sel(db, sql, params::Coll) = rowtable(exec(db, sql, params))
sel(db, sql, params...) = rowtable(exec(db, sql, params))
