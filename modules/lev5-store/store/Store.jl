...
function dbChecks()
    ...
    rows = select("select oid, orderType, primitDir, prillDir from Ord where (orderType = 3 and (primitDir <= 0 or prillDir <= 0)) or (orderType = 2 and (primitDir >= 0 or prillDir >= 0))")
